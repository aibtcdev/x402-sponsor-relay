...
import type { RateLimitTier, ApiKeyMetadata, ApiKeyUsage } from "../src/types";
...
function listKeys(): void {
  ...
  console.log(
    "Key ID          | App Name           | Tier      | Active | Expires"
  );
  console.log(
    "----------------+--------------------+-----------+--------+----------------------"
  );
...
async function getKeyInfo(apiKey: string): Promise<void> {
  ...
  console.log(
    `Status: ${!metadata.active ? "Revoked" : isExpired ? "Expired" : "Valid"}`
  );
  console.log(`\nRate Limits (${metadata.tier} tier):`);
  console.log(
    `  - ${TIER_LIMITS[metadata.tier].requestsPerMinute} requests/minute`
  );
...
async function getKeyUsage(apiKey: string, days: number = 7): Promise<void> {
  ...
  console.log(
    "Date       | Requests | Success | Failed | STX Volume    | Fees Paid"
  );
  console.log(
    "-----------+----------+---------+--------+---------------+-------------"
  );
  ...
  for (let i = 0; i < days; i++) {
    ...
    const usage = kvGet<ApiKeyUsage>(
      `usage:daily:${metadata.keyId}:${dateStr}`
    );
  ...
  switch (command) {
    case "create": {
      if (!args.app || !args.email) {
        ...
        console.error(
          "Usage: npm run keys -- create --app <name> --email <email>"
        );
        process.exit(1);
      }
      ...
      if (!VALID_TIERS.includes(tier as RateLimitTier)) {
        console.error(
          `Error: Invalid tier "${tier}". Valid tiers are: ${VALID_TIERS.join(", ")}`
        );
        process.exit(1);
      }
      ...
      if (!VALID_ENVS.includes(env as "test" | "live")) {
        console.error(
          `Error: Invalid env "${env}". Valid environments are: ${VALID_ENVS.join(", ")}`
        );
        process.exit(1);
      }
