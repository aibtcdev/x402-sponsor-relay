...
describe("toSponsorStatusResult", () => {
  ...
  it("keeps snapshots healthy through the 5 minute cache window", () => {
    const result = toSponsorStatusResult(
      makeSnapshot(),
      Date.parse("2026-03-30T18:24:30.000Z")
    );
  ...
  it("marks expired snapshots unavailable", () => {
    const result = toSponsorStatusResult(
      makeSnapshot(),
      Date.parse("2026-03-30T18:31:00.000Z")
    );
...
describe("sponsor status freshness helpers", () => {
  it("treats snapshot freshness windows as fresh for 5 minutes, then stale, then expired", () => {
    expect(
      getSponsorSnapshotFreshness(
  ...
  it("treats reconciliation freshness windows as fresh for 5 minutes, stale after, or unavailable", () => {
    expect(
      getSponsorReconciliationFreshness(
        "2026-03-30T18:19:30.000Z",
        Date.parse("2026-03-30T18:24:00.000Z")
      )
    ).toBe("fresh");
    expect(
      getSponsorReconciliationFreshness(
...
describe("SponsorStatus schema", () => {
  ...
  it("documents unavailable responses with the full cached status shape", () => {
    const endpoint = new SponsorStatus();
    const properties =
    ...
    expect(properties).toHaveProperty("noncePool");
    expect(properties).toHaveProperty("reconciliation");
    expect(properties.snapshot.properties.freshness.enum).toEqual(["expired"]);
  });
});
