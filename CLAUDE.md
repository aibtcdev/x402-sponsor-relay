...
- `GET /api-docs` - Swagger UI API documentation (Chanfana)
...
- `GET /docs` - Topic index: JSON array of available topic docs with descriptions and URLs
- `GET /docs/:topic` - Topic sub-docs (plaintext). Available topics:
...
Note: Chanfana Swagger UI was moved from `/docs` to `/api-docs` to keep `/docs` available for the AX JSON topic index.
...
**Key Files:**
- `src/index.ts` - Hono app entry point with Chanfana OpenAPI setup (Swagger at /api-docs)
- `src/routes/discovery.ts` - AX discovery routes (/llms.txt, /llms-full.txt, /docs, /docs/:topic, /.well-known/agent.json)
